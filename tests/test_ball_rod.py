...
class BallRodTest(unittest.TestCase):
    ...
    def test_ball_moves_into_stationary_rod(self):
    ...
    def test_stationary_vertical_rod_and_incoming_ball_energy(self):
    ...
    def test_potential_zero_and_percussion_center(self):
        ...
        self.assertAlmostEqual(model.sliders["L"].value, 0.5, places=12)
        self.assertAlmostEqual(model.percussion_center(), 2.0 * 0.5 / 3.0, places=12)
    ...
    def test_percussion_center_has_zero_pivot_impulse(self):
        model = self.make_model(vc=3.0, L=0.45, h=0.30)
    ...
    def test_high_ball_speed_does_not_prelaunch_rod(self):
        model = self.make_model(vc=10.0, h=0.8, g=9.8)
        self.assertFalse(model.uses_initial_speed)
        self.assertAlmostEqual(model.initial_angle_deg, 0.0, places=12)
        self.assertEqual(model.initial_omega, 0.0)
        self.assertEqual(model.target_omega, 0.0)

    def test_zero_gravity_still_uses_incoming_ball(self):
    ...
    def test_zero_ball_speed_does_not_fake_a_collision(self):
        model = self.make_model(vc=0.0, g=0.0)
        self.assertAlmostEqual(model.initial_angle_deg, 0.0, places=12)
        self.assertEqual(model.initial_omega, 0.0)
        model.jump_to_collision()
        self.assertFalse(model.collided)
        self.assertIn("速率为 0", model.notice)
    ...
    def test_target_control_is_ball_speed(self):
        model = self.make_model(h=0.8, omega_c=5.0)
        self.assertAlmostEqual(model.target_ball_speed(), 5.0, places=12)
        self.assertAlmostEqual(model.target_angular_speed(), 0.0, places=12)
        self.assertAlmostEqual(model.target_collision_speed(), 0.0, places=12)
        ...
        self.assertAlmostEqual(model.target_ball_speed(), 3.0, places=12)

    def test_rod_length_scale_is_zero_to_half_meter(self):
        model = BallHitsRod()
        slider = model.sliders["L"]
        self.assertEqual(slider.vmin, 0.0)
        self.assertEqual(slider.vmax, 0.5)
        model.set_control_value("L", 0.0)
        model.reset()
        model.start_pause()
        self.assertFalse(model.running)
        self.assertIn("L=0", model.notice)
