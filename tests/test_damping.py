...
class DampingTest(unittest.TestCase):
    ...
    def test_zero_damping_preserves_post_collision_energy(self):
        model = self.make_model(vc=3.0, h=0.72, b=0.0)
        model.jump_to_collision()
    ...
    def test_positive_damping_reduces_energy_and_records_loss(self):
        model = self.make_model(vc=3.0, h=0.72, b=0.05)
        model.jump_to_collision()
        initial = model.mechanical_energy()
    ...
    def test_energy_account_closes_after_damped_motion(self):
        model = self.make_model(vc=3.0, h=0.72, e=0.6, b=0.05)
        model.jump_to_collision()
        for _ in range(480):
            model.step(1.0 / 240.0)
    ...
    def test_damping_does_not_change_instantaneous_collision(self):
        no_damping = self.make_model(vc=3.0, h=0.72, b=0.0)
        damping = self.make_model(vc=3.0, h=0.72, b=0.05)
        no_damping.jump_to_collision()
        damping.jump_to_collision()

        for key in ("v_after", "omega_after", "impulse", "energy_after"):
            self.assertAlmostEqual(no_damping.last_result[key], damping.last_result[key], places=12)

    def test_collision_explanation_freezes_physics_until_complete(self):
        model = self.make_model(vc=3.0, h=0.72, b=0.02)
