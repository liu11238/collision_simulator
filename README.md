...
- 双球一维碰撞（可调恢复系数）
- 小球水平入射撞击初始静止的定轴细杆，杆长可在 0--0.5 m 范围设置
...
第二个模型中，细杆初始竖直向下且保持静止，小球从左侧水平向右入射撞杆。
界面输入的 `u` 是小球入射速率，程序内部令其速度为 `v=u`；碰撞前杆的
角速度为 `ω=0`。杆长 `L` 的比例尺为 `0--0.5 m`，其中 `L=0` 仅作为
比例尺端点，仿真时需要将 `L` 和碰撞高度 `h` 设置为大于零。

碰撞瞬间使用

```text
I = M L^2 / 3                 w = 0
J = -(1+e)(v-hw)/(1/m+h^2/I)
```

恢复系数 `e` 可调（`e=1` 为完全弹性碰撞），满足
